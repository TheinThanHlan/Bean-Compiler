...
def generate_class(tokens):
    ...
    ANNOTATIONS = "".join(
        tokens.get("ANNOTATIONS", {}).get("BEAN", {}).get(config.postfix, [])
    )
    ...
    output = CLASS_FORMAT.format(
        ANNOTATIONS=ANNOTATIONS,
        CLASS=tokens.get("CLASS"),
        CLASS_BODY=CLASS_BODY,
        IMPLEMENTS=IMPLEMENTS,
        EXTENDS=EXTENDS,
        IS_THERE_EXTENDS=IS_THERE_EXTENDS,
        IS_THERE_IMPLEMENTS=IMPLEMENTS,
    )
    return output
...
def generate_variables(tokens):
    VARIABLE_FORMAT = (
        "{ANNOTATIONS} public {NAME}{HAVE_DEFAULT}:{TYPE}{IS_ARR}{DEFAULT};"
    )
    output = ""
    for a in tokens.get("VARIABLES"):
        ANNOTATIONS = "".join(
            a.get("ANNOTATIONS", {}).get("BEAN", {}).get(config.postfix, [])
        )
...
def generate_equals(tokens):
    ...
    COMPARISONS_PRIMITIVE_FORMAT = (
        "if(this.get{COMPARE_VAR}()!=obj1.get{COMPARE_VAR}()){{return false;}}"
    )
    ...
    output = ""
    for a in tokens.get("VARIABLES"):
        COMPARISONS += COMPARISONS_PRIMITIVE_FORMAT.format(
            COMPARE_VAR=utils.makeFuncCase(a.get("NAME"))
        )
